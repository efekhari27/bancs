...
#
#Possible choices:  "AMISE", "Beta", "LogLikelihood" and "PenalizedKL".
ebc_tuning = "LogLikelihood"
nb_reps = 10
bench_sizes = [300, 500, 700, int(1e3), int(2e3), int(5e3)]

## RP parabolic ##
#####################
rb = ReliabilityBenchmark(problems=[problem_parabolic], methods=["BANCS", "SS", "NAIS"], sizes=bench_sizes, ebc_tuning=ebc_tuning, reps=nb_reps)
rb.run(save_file=f"bancs_results/Parabolic_results_{ebc_tuning}.csv")

## RP 53 ##
#####################
#rb = ReliabilityBenchmark(problems=[problemRP53], methods=["BANCS", "SS", "NAIS"], sizes=bench_sizes, ebc_tuning=ebc_tuning, reps=nb_reps)
#rb.run(save_file=f"bancs_results/RP53_results_{ebc_tuning}.csv")

## RP 38 ##
#####################
#rb = ReliabilityBenchmark(problems=[problemRP38], methods=["BANCS", "SS", "NAIS"], sizes=bench_sizes, ebc_tuning=ebc_tuning, reps=nb_reps)
#rb.run(save_file=f"bancs_results/RP38_results_{ebc_tuning}.csv")

## RP four-branch ##
#####################
#rb = ReliabilityBenchmark(problems=[problem4B], methods=["BANCS", "SS", "NAIS"], sizes=bench_sizes, ebc_tuning=ebc_tuning, reps=nb_reps)
#rb.run(save_file=f"bancs_results/RP4B_results_{ebc_tuning}.csv")

## Oscillator ##
################
# rb = ReliabilityBenchmark(problems=[oscillator_problem], methods=["BANCS", "SS"], sizes=bench_sizes, ebc_tuning=ebc_tuning, reps=nb_reps)
# rb.run(save_file=f"bancs_results/Oscillator_results_{ebc_tuning}.csv")


# salloc --nodes=1 --cpus-per-task=36 --mem=16G --time=60 --wckey=p1279:openturns srun python benchmark.py
# salloc --nodes=1 --cpus-per-task=36 --mem=16G --time=10 --wckey=p1279:openturns srun myscript.sh
