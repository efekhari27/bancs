...
This code is a Python prototype of the openturns.NAIS class developed by M. Balesdent in 2022. 
The present version adds the possibility to define a truncated domain to the inputs.
"""
...
class NAISAlgorithm(object):
    def __init__(self,event,n_IS,rho_quantile, lower_truncatures=[None], upper_truncatures=[None]):
    ...
    def compute_aux_distribution(self, sample, weights):
        ...
        silverman = sample.computeStandardDeviation()*(neff * (self.dim + 2) / 4.)**(-1. / (self.dim + 4))
        ...
        for k in range(self.dim):
            ...
            # Truncation
            if (self.lower_truncatures[k] != None) and (self.upper_truncatures[k] != None):
                distri_margin = ot.TruncatedDistribution(distri_margin, self.lower_truncatures[k], self.upper_truncatures[k])
            elif self.lower_truncatures[k] != None: 
                distri_margin = ot.TruncatedDistribution(distri_margin, self.lower_truncatures[k], ot.TruncatedDistribution.LOWER)
            elif self.upper_truncatures[k] != None: 
                distri_margin = ot.TruncatedDistribution(distri_margin, self.upper_truncatures[k], ot.TruncatedDistribution.UPPER)      
